...
## Лёгкий запуск и центрирование

- Добавлен launcher.py: проверка готовности приложения/Ollama, скрытый запуск недостающих процессов, межпроцессная блокировка, открытие браузера, сообщение об ошибке.
- Ярлык создан через scripts/create_shortcut.ps1 на рабочем столе и запущен. Реально проверены холодный старт приложения и повторное использование того же PID. Работающую Ollama не останавливали; сценарий запуска отсутствующей Ollama проверен с подменой процессов.
- Центрирование на viewport 1920 px: ширина контейнера 1480 px, левый и правый отступы 220 px. Пояснения Advanced analysis проверены в раскрытом разделе.
- Актуальный набор: **68 Python-тестов и 2 JS-теста**. Закрытие браузера по-прежнему не останавливает скрытый сервер; трей и команда завершения не реализованы.
